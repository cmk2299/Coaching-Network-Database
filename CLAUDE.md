...
- **build_sqlite Namespace-Crash (2026-06-05):** `import_persons` nutzte `p.get("tm_id", int(f.stem))` — der Default-Arg wird IMMER evaluiert, und `int()` crasht auf namespaced Profil-Stems (`spieler_10`, `trainer_X`) aus der Namespace-Migration. Folge: Build scheiterte seit der Migration an der persons-Stufe → `data/coaches.db` blieb bei 503 KB mit **persons=0**. Fix: tm_id zuerst aus JSON lesen, dann auf numerischen Tail des Stems (nach Strip von `trainer_`/`spieler_`) zurückfallen. Rebuild: 48,9 MB / 84.819 persons / 61.225 career / 283.619 squad_entries. Gleiches Root-Pattern wie der `preload_all_profiles`-Namespace-Crash (Memory 2026-05-22) — bei künftigen Profil-Iterierern IMMER namespaced Stems erwarten. `data/coaches.db` ist Backend-Artefakt (api.py/coach_db.py), wird NICHT via Vercel served → nicht ins output/, nicht ins git (48 MB, regenerierbar).
